//*CID://+1Ah0R~:                                   update#=  15;  //~1Ah0R~
//***********************************************************************//~v1B6I~
//1Ah0 2016/09/26 bonanza                                          //~1Ah0I~
...
import static com.Asgts.gtp.GoColor.BLACK;                         //~1AfmR~
import static com.Asgts.gtp.GoColor.WHITE;                         //~1AfmR~
...
//  public String getCommandGenmove(GoColor color)                 //~v1B6R~
    public String getCommandGenmove(GoColor color,String Preg_genmove)//~1AfmR~
    {
      ...
      if (Preg_genmove!=null)                                      //~1AfmR~
      {                                                            //~1AfmR~
        if (color == BLACK)                                        //~1AfmR~
            return Preg_genmove+ " b";                             //~1AfmR~
        else                                                       //~1AfmR~
            return Preg_genmove+ " w";                             //~1AfmR~
      }                                                            //~1AfmR~
      else                                                         //~1AfmR~
        if (color == BLACK)
            return "genmove b";
    ...
    public String getCommandPlay(GoColor Pcolor,String Pcsamove)   //~1Ah0I~
    {                                                              //~1Ah0I~
        m_buffer.setLength(0);                                     //~1Ah0I~
        if (Pcolor == BLACK)                                       //~1Ah0I~
        	m_buffer.append("BLACK ");                             //~1Ah0I~
        else                                                       //~1Ah0I~
		if (Pcolor == WHITE)                                        //~1Ah0I~
        	m_buffer.append("WHITE ");                             //~1Ah0I~
        m_buffer.append(Pcsamove);                                 //~1Ah0I~
        return m_buffer.toString();                                //~1Ah0I~
    }                                                              //~1Ah0I~
    ...
    {
    	if (Dump.Y) Dump.println("GtpClientBase:isSupported cmd="+command);//~v1B6I~
        if (m_supportedCommands == null)
        {                                                          //~v1B6I~
    		if (Dump.Y) Dump.println("GtpClientBase:isSupported m_supportedCommand :null");//~v1B6I~
            return false;
        }                                                          //~v1B6I~
        for (int i = 0; i < m_supportedCommands.length; ++i)
            if (m_supportedCommands[i].equals(command))
            {                                                      //~v1B6I~
	    		if (Dump.Y) Dump.println("GtpClientBase:isSupported m_supportedCommand return true");//~v1B6I~
                return true;
            }                                                      //~v1B6I~
	    if (Dump.Y) Dump.println("GtpClientBase:isSupported m_supportedCommand return false");//~v1B6I~
        return false;
    }
    ...
    public void queryName()
    {
        try
        {
        	if (Dump.Y) Dump.println("GtpClientBase:queryName Start");//~1Ah0I~
            m_name = send("name");
        }
        catch (GtpError e)
        {
	        Dump.println(e,"GtpClientBase:queryName");             //~1Ah0I~
        }
        if (Dump.Y) Dump.println("GtpClientBase:queryName End");   //~1Ah0I~
    }
    ...
    public abstract String send(String command,boolean Psendplay) throws GtpError;//+1Ah0I~
    ...
    public void sendPlay(GoColor Pcolor,String Pcsamove) throws GtpError//~1Ah0I~
    {                                                              //~1Ah0I~
        send(getCommandPlay(Pcolor,Pcsamove));                      //~1Ah0I~
    }                                                              //~1Ah0I~
    public void sendPlay(String Pcsamove) throws GtpError          //~1Ah0I~
    {                                                              //~1Ah0I~
        if (Dump.Y) Dump.println("BtpClientBase:sedPlay cmd="+Pcsamove);//~1Ah0I~
        send(Pcsamove,true/*sendplay cmd*/);                       //~1Ah0R~
    }                                                              //~1Ah0I~
...
//    public abstract void waitForExit();                          //~v1B6R~
